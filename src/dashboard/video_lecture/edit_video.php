...
$id = $_POST['video_id'];
$sql = "SELECT * FROM video_lecture WHERE video_id='$id' AND uploaded_by = '$usn'";
...
<div class="conatiner" id="formConatinerVideo">
  ...
  <input type="text" value="<?php echo $data['video_topic'] ?>" class="form-control" id="topic" name="topic" placeholder="Enter topic with chapter name )" required>
        <div id="preview-video" style="display:block;">

        <video style="width:100%;background:#f9f9f9;" controls>
            <source src="src/dashboard/video_lecture/<?php echo $data['video_src'] ?>" id="videoTag">
        </video>
...
<input type="text" name="video_id" value=<?php echo $id; ?> style="display:none;">

  <textarea class="form-control"  id="aboutLecture" rows="3" placeholder="Write about lecture" name="about"><?php echo $data['video_about'] ?></textarea>
  ...
  <a role="button" id="uploadVideo" style="padding:8px 12px;border:1px solid #ddd;outline:none;text-decoration:none;color">Update</a>
...
$("document").ready(function(){
 $("#cancelUpdation").click(function(){
    ...
    function sendData(){
        ...
        $.ajax({
            url:"src/dashboard/video_lecture/edit_video_bck.php",
            type:"post",
            data:Data,
            contentType:false,
             processData:false,
